...
def TerceiraJanela():
    ...
    Query=Dado.query.filter_by(NomeDaEmitente = current_user.NomeDaEmpresaDB).order_by(Dado.id.desc()).limit(10).all()
    ContadorGrupo= Dado.query.with_entities(Dado.MarcaDB,Dado.ModeloDB,Dado.AnoDB,func.count(Dado.ModeloDB).label('total')).group_by(Dado.ModeloDB).order_by(desc('total')).limit(20)
    ContadorGrupo2= Dado.query.with_entities(Dado.MarcaDB,func.count(Dado.MarcaDB).label('total2')).group_by(Dado.MarcaDB).order_by(desc('total2')).limit(20).all()
    ContadorGrupo3= Dado.query.with_entities(Dado.MarcaDB,Dado.ModeloDB,Dado.AnoDB,func.avg(Dado.MargemDeLucroDB).label('media')).group_by(Dado.ModeloDB).order_by(desc('media')).all()
    faltante= 5-contador
    if contador is None or contador < 5:
        ...
        return render_template("TerceiraJanela.html", title = "TerceiraJanela",PresentePython= PresentePython, 
                                TituloDadosInserido =TituloDadosInserido,Query=Query,
                                TituloTop20Modelo= TituloTop20Modelo, ContadorGrupo= ContadorGrupo,
                                TituloTop20Marca=TituloTop20Marca, ContadorGrupo2= ContadorGrupo2,
                                TituloMargemLucro= TituloMargemLucro, ContadorGrupo3= ContadorGrupo3)
